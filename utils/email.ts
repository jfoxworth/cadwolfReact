...
export async function sendVerificationEmail(to: string, verifyUrl: string) {
  await transporter.sendMail({
    from: FROM,
    to,
    subject: "Verify your Engentic email address",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;">
        <h2 style="margin:0 0 8px;font-size:22px;color:#111;">Verify your email</h2>
        <p style="color:#555;margin:0 0 24px;">Click below to confirm your Engentic account. This link expires in 24 hours.</p>
        <a href="${verifyUrl}"
        ...
        <p style="color:#888;font-size:13px;margin:24px 0 0;">
          If you didn't create an Engentic account, you can ignore this email.<br>
...
export async function sendTeamInviteEmail(to: string, inviteUrl: string, teamName: string, role: string) {
  await transporter.sendMail({
    from: FROM,
    to,
    subject: `You've been invited to join ${teamName} on Engentic`,
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;">
...
export async function sendEmailChangeEmail(to: string, confirmUrl: string) {
  await transporter.sendMail({
    from: FROM,
    to,
    subject: "Confirm your new Engentic email address",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;">
        <h2 style="margin:0 0 8px;font-size:22px;color:#111;">Confirm your new email</h2>
        <p style="color:#555;margin:0 0 24px;">Click below to confirm this as your new Engentic email address. This link expires in 1 hour.</p>
        <a href="${confirmUrl}"
           style="display:inline-block;background:#2563eb;color:#fff;text-decoration:none;padding:12px 24px;border-radius:8px;font-size:15px;font-weight:600;">
...
export async function sendPasswordResetEmail(to: string, resetUrl: string) {
  await transporter.sendMail({
    from: FROM,
    to,
    subject: "Reset your Engentic password",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;">
