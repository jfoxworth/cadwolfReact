...
export default function SelectBlock({
  ...
  const def = block.definition as SelectBlockDef;

  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState<SelectBlockDef>({ ...def });
  ...
  return (
    <div
      ...
      className={[
        "flex flex-col gap-1.5 rounded px-3 py-2",
    ...
    >
      {def.variableName && (
        <span className="font-mono text-xs font-medium text-gray-500">{def.variableName}</span>
      )}
      <div className="flex flex-col gap-1">
        {options.map((opt, idx) => (
          ...
          >
            <input
              type="radio"
              name={`select-${block.id}`}
              checked={selectedIndex === idx}
              onChange={() => handleRadioChange(idx)}
              className="accent-blue-600"
            />
            <span className="font-mono text-sm text-gray-700">{opt}</span>
          </label>
        ))}
      </div>
    </div>
