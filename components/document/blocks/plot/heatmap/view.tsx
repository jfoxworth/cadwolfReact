...
/** Convert solver Matrix (flat key dict + size) to a 2D row-major array. */
...
/** Convert solver Matrix to a flat 1D array (row or column vector). */
...
export default function HeatmapView({ def, solverResults, revision }: Props) {
  const height     = def.height ?? 400;
  const colorscale = def.colorscale ?? "Viridis";

  // zVar holds the 2D matrix variable name; xVar/yVar hold optional tick label vectors
  ...
  const trace: Record<string, unknown> = {
    ...
    ...(yTicks ? { y: yTicks } : {}),
    ...(def.showNumbers
      ? { text: z.map((row) => row.map(String)), texttemplate: "%{text}", textfont: { size: 10 } }
      : {}),
  };
  ...
  return (
    <Plot
      ...
      layout={{
        ...
        xaxis: { title: { text: def.xLabel || def.hmXVar || "" }, automargin: true },
