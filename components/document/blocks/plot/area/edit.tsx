...
import type { PlotDefinition, PlotSeries, AreaMode } from "../types";
...
export default function AreaEdit({ draft, varNames, onChange }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <label className="flex flex-col gap-1">
        <span className="text-xs font-medium text-gray-600">Stacking</span>
        <select
          value={draft.areaMode ?? "standard"}
          onChange={(e) => onChange({ areaMode: e.target.value as AreaMode })}
          className="rounded border border-gray-300 px-2 py-1.5 text-sm focus:border-blue-400 focus:outline-none bg-white"
        >
          <option value="standard">Standard (each series fills to zero)</option>
          <option value="stacked">Stacked</option>
          <option value="normalized">Stacked — normalized to 100%</option>
        </select>
      </label>
