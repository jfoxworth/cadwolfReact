...
export default function AreaView({ def, solverResults, revision }: Props) {
  const series  = def.series ?? [];
  const height  = def.height ?? 400;
  const mode    = def.areaMode ?? "standard";
  const colorway = resolveColors(def.colorScheme);

  // Plotly stacking: stackgroup groups traces; groupnorm normalises them.
  const stackgroup  = mode !== "standard" ? "one" : undefined;
  const groupnorm   = mode === "normalized" ? ("percent" as const) : undefined;
  ...
  if (traces.length === 0) {
    return (
      <div
        ...
        style={{ height }}
      >
        <span>Area chart — no series defined</span>
      </div>
    );
  }
