...
export default function RegisterPage() {
  ...
  const { state: usernameState, errorMsg: usernameError } = useUsernameCheck(username);
  ...
  const usernameStatusColor =
    usernameState === "available" ? "text-green-600" :
    usernameState === "taken" || usernameState === "invalid" ? "text-red-600" :
    "text-gray-400";

  const usernameStatusText =
    usernameState === "available" ? "Available!" :
    usernameState === "checking" ? "Checking…" :
    usernameState === "taken" ? "Already taken" :
      ...
      <div className="bg-white rounded-xl shadow-md w-full max-w-sm p-8">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            />
            {usernameStatusText && (
              <p className={`text-xs mt-1 ${usernameStatusColor}`}>{usernameStatusText}</p>
            )}
