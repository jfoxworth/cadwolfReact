...
describe("percentile — unit preservation (inline units)", () => {
  ...
  it("percentile([3,5,7] m, 50) → preserves meter dimension", async () => {
    const res = await solveVecUnit("x = [3, 5, 7] m", "y = percentile(x, 50)");
    expect(res?.solution?.baseUnits?.some(v => v !== 0)).toBe(true);
  });
  it("percentile([3,5,7] kg*m/s^2, 50) → preserves combined units", async () => {
    const res = await solveVecUnit("x = [3, 5, 7] kg*m/s^2", "y = percentile(x, 50)");
    expect(res?.solution?.baseUnits?.some(v => v !== 0)).toBe(true);
  });
  it("percentile([3,5,7] km, 50) → preserves scaled unit", async () => {
    const res = await solveVecUnit("x = [3, 5, 7] km", "y = percentile(x, 50)");
    expect(res?.solution?.baseUnits?.some(v => v !== 0)).toBe(true);
  });
  it("percentile([25,50,75] kN, 50) → preserves complex scaled unit", async () => {
    const res = await solveVecUnit("x = [25, 50, 75] kN", "y = percentile(x, 50)");
    expect(res?.solution?.baseUnits?.some(v => v !== 0)).toBe(true);
  });
});
